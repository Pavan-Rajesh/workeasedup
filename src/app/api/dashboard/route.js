...
WHERE id::uuid IN (
    ...
    return NextResponse.json({ ownerdata, workersdata, headsData });
  } else if (role == "worker") {
    const workerdata =
    ...
    const headData =
      await client`select worktype,noofworkers,name,status from head where id=${id}`;
    const workersdata =
    ...
    return NextResponse.json({ headData, workersdata, ownerdata });
  }
}
